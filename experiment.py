...
def experiment(
    ...
    def eval_episodes(target_rew):
        def fn(model, critic, action_num, out_action_num):
            returns, lengths = [], []
            ...
            return {  # eval了10次, 然后取得均值
                f'target_{target_rew}_return_mean': np.max(returns),
                f'target_{target_rew}_return_std': np.std(returns),
                f'target_{target_rew}_length_mean': np.mean(lengths),
                f'target_{target_rew}_length_std': np.std(lengths),
                f'target_{target_rew}_normalized_score': env.get_normalized_score(np.max(returns)),
            }
    ...
    for iter in range(variant['max_iters']):
        outputs = trainer.train_iteration(num_steps=variant['num_steps_per_iter'], logger=logger,
                                          iter_num=iter + 1, log_writer=writer, action_num=action_num, max_uncertainty=max_uncertainty)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--grad_norm", default=15.0, type=float)
    ...
    # TODO cmd
    parser.add_argument("--action_num", default=20, type=int)
