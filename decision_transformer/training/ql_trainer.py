...
class Trainer:
    ...
    def train_iteration(self, num_steps, logger, iter_num=0, log_writer=None, action_num=10, max_uncertainty=0.8):
        ...
        for _ in trange(num_steps):
            loss_metric, out_action_num = self.train_step(log_writer, loss_metric, action_num, max_uncertainty)
        ...
        for eval_fn in self.eval_fns:
            outputs = eval_fn(self.actor, self.critic_target, action_num, out_action_num)
    ...
    def train_step(self, log_writer=None, loss_metric={}, action_num=10, max_uncertainty=0.8):
        '''
        ...
        def compute_out_action_num(pre_states, true_states, max_action_num, max_uncertainty):
            ...
            out_action_num = int(torch.clamp(1 + (max_action_num - 1) * (uncertainty / max_uncertainty), 1, max_action_num))
            return out_action_num

        next_states, pre_rewards, info = self.dynamics.step(states,
                                                                       actions)  # TODO 这里的reward是已经减完惩罚项penalty以后的了, 我其实只需要这个penalty就可以了
        next_states = torch.from_numpy(next_states).to(states)
        pre_rewards = torch.from_numpy(pre_rewards).to(rewards)
        out_action_num = compute_out_action_num(next_states, states, action_num, max_uncertainty)
        ...
        if self.max_q_backup:  # 不太懂这段是干嘛的..这个默认也是False, 可能论文里有解释吧..
