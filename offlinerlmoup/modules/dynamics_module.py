...
class EnsembleDynamicsModel(nn.Module):
    def __init__(
        ...
        num_ensemble: int = 7,
        num_elites: int = 5,
        activation: nn.Module = Swish,  # 激活函数选的是swish, 第一次听说这种激活函数
...
class DEnsembleDynamicsModel(nn.Module):
    def __init__(
        ...
        if weight_decays is None:
            weight_decays = [0.0] * (len(hidden_dims) + 1)
        for in_dim, out_dim, weight_decay in zip(hidden_dims[:-1], hidden_dims[1:], weight_decays[:-1]):  # 突然参透为什么要这么设计了, 其实就是一个窗口长度为2的滑动窗口吧
            module_list.append(EnsembleLinear(in_dim, out_dim, num_ensemble, weight_decay))  # 集成学习, 包括num_ensemble个线形层, 每个层有自己的参数?
            module_list.append(torch.nn.Dropout(p=0.01))
            module_list.append(torch.nn.LayerNorm(out_dim))
        self.backbones = nn.ModuleList(module_list)

        self.output_layer = EnsembleLinear(  # 输出把滑动窗口没滑到的最后一个取出来, 然后输出维度我们设计
            hidden_dims[-1],
            2 * (obs_dim + self._with_reward),  # 这里为什么要乘2呢?
            num_ensemble,
            weight_decays[-1]
        )
    ...
    def forward(self, obs_action: np.ndarray) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        mean, logvar = torch.chunk(pre_results_men, 2, dim=-1)  # 知道前面输出为啥要乘2了, 这里把输出分成两块, 一块来作为均值, 一块来作为对数方差
