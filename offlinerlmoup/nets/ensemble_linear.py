...
class EnsembleLinear(nn.Module):
    def __init__(
        ...
        # self.register_parameter("dt_bias", nn.Parameter(torch.zeros(num_ensemble, 1, 1, output_dim)))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if len(x.shape) == 2:
            ...
            if x.shape[0] == weight.shape[0]:
                ...
                x = torch.einsum('ijk,bko->bijo', x, weight)
                bias = bias.unsqueeze(2)
        elif len(x.shape) == 4:
            x = torch.einsum('bijo,bok->bijk', x, weight)
            bias = bias.unsqueeze(2)
        # if len(x.shape) == 3:
        x = x + bias
        # elif len(x.shape) == 4:
